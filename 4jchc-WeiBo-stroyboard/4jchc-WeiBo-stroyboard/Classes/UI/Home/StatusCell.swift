...
//            sourceLabel.text = status?.source
            ...
            if let iconUrl = status?.user?.profile_image_url {
                NetworkManager.sharedNetworkManager.requestImage(iconUrl) { (result, error) -> () in
...
extension StatusCell: UICollectionViewDataSource,UICollectionViewDelegate {
    ...
    */
    func calcPictureViewSize() -> (itemSize: CGSize, viewSize: CGSize) {
        ...
        if count == 1 {
            // 拿到被缓存的图像
            let path = NetworkManager.sharedNetworkManager.fullImageCachePath(status!.pictureUrls![0].thumbnail_pic!)
            // 实例化图像 － 图像有可能下载失败
...
class PictureCell: UICollectionViewCell {
    ...
    var urlString: String? {
        didSet {
            // 1. 图像缓存路径
            let path = NetworkManager.sharedNetworkManager.fullImageCachePath(urlString!)
            // 2. 实例化图像
